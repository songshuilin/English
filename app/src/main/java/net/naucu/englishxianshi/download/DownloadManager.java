...
public final class DownloadManager {
    ...
    /**
     * changed by fght at 16.1.14
     */
    public synchronized void removeDownload(int index, boolean deleteFile) throws DbException {
        DownloadInfo downloadInfo = downloadInfoList.get(index);
        stopDownload(downloadInfo);
        ...
        if (deleteFile) {
            ...
            if (downloadInfo.getState() == DownloadState.FINISHED) {
                file = new File(downloadInfo.getFileSavePath());
                if (file.exists()) {
                    file.delete();
                }
            } else {
                file = new File(downloadInfo.getFileSavePath() + ".tmp");
                if (file.exists()) {
                    file.delete();
                }
            }
     ...
     * changed by fght at 16.1.14
     */
    public synchronized void removeDownload(DownloadInfo downloadInfo, boolean deleteFile) throws DbException {
        stopDownload(downloadInfo);
        db.delete(downloadInfo);
        downloadInfoList.remove(downloadInfo);
    }
